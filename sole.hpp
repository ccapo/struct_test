...
namespace sole {
    ...
    inline std::string uuid::raw() const {
        std::stringstream ss;
        ...
        version <= 1 && ss << printftime(timestamp/10000000) << ',';
        version <= 1 && ss << std::setw(4) << (c & 0xFFFF) << std::setw(8) << d << ',';
        version == 4 && ss << std::setw(8) << (ab & 0xFFFFFFFFFFFF0FFFULL) << std::setw(8) << (cd & 0x3FFFFFFFFFFFFFFFULL);

        ss << std::dec;
        version == 0 && ss << std::setw(4) << (c >> 16 ) << ',';
        version == 1 && ss << std::setw(4) << ((c >> 16) & 0x3FFF) << ',';
